...
//    LOG << "Adjustment_counter" << Adjustment_counter << std::endl;
    ...
    switch(transition_state){
        ...
        case GOAL_NOT_REACHED:
            ...
            if((X <= (Rover_Goal_X[0] + 0.1) and X >= (Rover_Goal_X[0] - 0.1)) and
              (Y <= (Rover_Goal_Y[0] + 0.1) and Y >= (Rover_Goal_Y[0] - 0.1)))
               {
                   transition_state = GOAL_REACHED;
               }
//            /* Only one of the coordinate is equal to one of the goal coordinate */
//            else if((X <= (Rover_Goal_X[0] + 0.1) and X >= (Rover_Goal_X[0] - 0.1)) or
//                   (Y <= (Rover_Goal_Y[0] + 0.1) and Y >= (Rover_Goal_Y[0] - 0.1)))
//            {
//
...
CRadians CFootBotThesis::CalculateHeadingAngle(CVector3 cPosition){
    ...
    dist_y = Rover_Goal_Y[0] - cPosition.GetY();
    dist_x = Rover_Goal_X[0] - cPosition.GetX();
...
void CFootBotThesis::SetHeadingAngle(CRadians cheadangle){
    ...
    /* continue to rotate at specified angle */
        ...
        if(bCheckCollision)
        {
            ...
            LOG<< "cHeading_angle: "<< cHeading_angle << std::endl;
            LOG<< "cZAngle: "<< cZAngle << std::endl;
...
/****************************************/
//void CFootBotThesis::void SetWheelSpeeds(const CVector2& c_heading);
...
//        if(Abs(chead_angle) <= SoftTurnOnAngleThreshold) {
//            TurningMechanism = SOFT_TURN;
//        }
//    }
//    if(TurningMechanism == SOFT_TURN) {
//        if(Abs(chead_angle) > HardTurnOnAngleThreshold) {
//            TurningMechanism = HARD_TURN;
//        }
//        else if(Abs(chead_angle) <= NoTurnAngleThreshold) {
//            TurningMechanism = NO_TURN;
//        }
//    }
...
//            Real fSpeedFactor = (HardTurnOnAngleThreshold - Abs(chead_angle)) / HardTurnOnAngleThreshold;
